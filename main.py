...
train_outputs = model(split_dataset[0].transpose(2, 1).float(), split_dataset[1].float()).view(-1, 6, predict_days).cpu().detach().numpy()
test_outputs = model(test[0].transpose(2, 1).float(), test[1].float()).view(-1, 6, predict_days).cpu().detach().numpy()
train_ans = split_dataset[4].cpu().detach().numpy()
test_ans = test[4].cpu().detach().numpy()
train_baseline = split_dataset[1].cpu().detach().numpy()
